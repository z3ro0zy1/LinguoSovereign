...
这些资源对产品体验是必要的，但**不适合**直接长期放在主 Git 仓库里，原因包括：

- 会显著拖慢 clone、fetch、pull、push
- 大文件会让 Git 历史迅速膨胀
- 仓库维护成本变高
- 某些题目资源还可能需要单独考虑分发与版权边界

### 推荐做法

建议采用“代码仓库 + 单独资源包”的分离方式：

- Git 仓库只保留代码、配置模板、文档
- 完整真题资源单独分发
- 开发者拿到资源包后，再手动放到本地 `public/` 目录中

### 本地目录预期

应用默认会去 `public/` 下找这些资源。
常见例子包括：
...
1. 克隆代码仓库。
2. 安装依赖。
3. 从 `.env.example` 复制一份 `.env` 并填写。
4. 向项目维护者获取单独的真题资源包。
5. 将资源包解压或复制到本地 `public/` 目录。
6. 启动项目。
...
第一步先检查的不是代码，而是本地 `public/` 资源包是否已经正确下载和放置。
